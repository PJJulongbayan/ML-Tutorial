# This lab demonstrates use of simple ensemble method, Random Forest, using the drug dataset. 
...
from sklearn.preprocessing import StandardScaler, LabelEncoder # for easier coding 
...
# rf_classifier - it shows the default parameters; note that we can use Optuna or Gridsearch for hyperparameter optimization

rf_classifier.fit(X_train,y_train)
predTree = rf_classifier.predict(X_test)

# print (predTree [0:5])
# print (y_test[0:5])

print("Preliminary accuracy: %0.2f" % accuracy_score(y_test, predTree))
...
def objective(trial):
    ...
    # Train DT model
    rf_classifier =RandomForestClassifier(n_estimators=100, max_depth=k, random_state=0)
    rf_classifier.fit(X_train,y_train)
...
rf_classifier =RandomForestClassifier(n_estimators=100, max_depth=best_params['max_depth'], random_state=0)
rf_classifier.fit(X_train,y_train)
...
print("Train set accuracy of the RF model: %0.2f" % accuracy_score(y_train, rf_classifier.predict(X_train)))
print("Test set accuracy of the RF model: %0.2f" % accuracy_score(y_test, yhat))
